...
server.listen(args.port || 3000, async function () {
  wLog(`Servidor SQL Alarmes -> Kafka online em: "http://${await getIPAddress()}:${args.port || 3000}"`);
});
...
function main(startp, endp, robotsp, kafkaHost, kafkaTopic, targetId) {
  ...
  if (targetId) {
    for (let index = 0; index < robotsJob.length; index++) {
      exec(["node", "robotAlarme", `--host=${kafkaHost}`, `--topic=${kafkaTopic}`, `--start=${robotsJob[index].start}`, `--end=${robotsJob[index].end}`, `--dispositivoId=${targetId}`], log.bind({ index: index + 1 }));

    }
  } else {
    for (let index = 0; index < robotsJob.length; index++) {
      exec(["node", "robotAlarme", `--host=${kafkaHost}`, `--topic=${kafkaTopic}`, `--start=${robotsJob[index].start}`, `--end=${robotsJob[index].end}`], log.bind({ index: index + 1 }));
